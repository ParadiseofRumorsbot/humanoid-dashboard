...
• 현대글로비스 → 생산·판매 물류`,
  ...
  technology: {
    ...
    /* ── 13. 핵심 기술 용어 사전 ── */
    glossary: [
      { term: 'VLA', full: 'Vision-Language-Action', def: '시각+언어 입력 → 로봇 행동 출력하는 트랜스포머 모델. 인터넷 규모로 사전학습한 VLM 백본을 소량 로봇 데이터로 fine-tune → 기종 바뀌어도 재학습 불필요. "로봇용 안드로이드 OS"', category: 'model' },
      { term: 'VLM', full: 'Vision-Language Model', def: '이미지와 텍스트를 동시에 이해하는 멀티모달 모델 (VLA의 백본)', category: 'model' },
      { term: 'WFM', full: 'World Foundation Model', def: '물리 세계의 미래 상태를 예측하는 대규모 모델 = 로봇의 "물리적 상상력". 학습에 텍스트 LLM 대비 ~90배 데이터 필요(Cosmos 9,000조 토큰). Cosmos·Genie·V-JEPA', category: 'model' },
      { term: 'Flow Matching', full: 'Flow Matching', def: '노이즈→행동으로 가는 "직선 경로(속도장)"를 통째로 학습. Diffusion보다 적은 스텝으로 부드럽고 빠른 동작 생성. Pi0의 액션 헤드 방식', category: 'method' },
      ...
      { term: 'Dual System', full: 'System 1 + System 2', def: 'System1=빠른 반사(10ms, 모터 제어), System2=느린 추론·계획. 인간 인지 구조 모방. Atlas는 BD 피지컬AI가 S1, 구글 Gemini가 S2 담당', category: 'architecture' },
      ...
      { term: 'Pi0 (파이제로)', full: 'Pi-Zero · Physical Intelligence', def: 'Physical Intelligence의 범용 VLA. 구글 SigLIP(시각)+Gemma(언어)=PaliGemma 백본 + Flow Matching 액션 헤드. 7종 로봇을 한 모델로 동시 학습. 정밀 조작 92%. 후속 Pi0.5/0.6', category: 'model' },
      { term: 'GR00T', full: 'NVIDIA Isaac GR00T', def: 'NVIDIA 대표 휴머노이드 VLA. N1→1.5→1.6→1.7 빠른 진화. Eagle/Cosmos VLM 사용. DreamGen 합성데이터+인간 1인칭 영상 학습. 2026.10 레퍼런스 로봇(Unitree H2 Plus+Jetson Thor) 출시', category: 'model' },
      { term: 'Cosmos 3.0', full: 'NVIDIA Cosmos', def: 'NVIDIA 물리 AI 파운데이션 모델(2026.06 발표). Mixture of Transformers 옴니 모델 — VLM·월드모델·시뮬레이터·정책을 하나로. "compute is data". 물리 AI 최전선', category: 'model' },
      { term: 'Alpamayo 2', full: 'NVIDIA Alpamayo', def: 'NVIDIA 자율주행 오픈 모델(2026.06). "세계 최초 추론형 AV". Hyperion(글로벌 차량 ~80%) 위에서 작동. GR00T와 같은 물리 AI 스택의 자동차 버전', category: 'model' },
      { term: 'DreamGen', full: 'DreamGen Pipeline', def: 'NVIDIA 합성 데이터 생성. 실 로봇 영상으로 비디오모델 fine-tune→안 본 동작 영상 생성→Inverse Dynamics로 액션 추산→VLA 학습. 실데이터 병목 우회', category: 'data' },
      { term: 'Latent Actions', full: 'Latent Action Learning (LAPA)', def: '인간 1인칭 영상을 학습에 쓰는 기법. 영상에서 잠재 액션 공간을 만들어 사람 영상을 "또 다른 로봇 데이터"로 변환', category: 'data' },
  ...
  updateLog: [
    {
      ...
      changes: [
        '구조 확정: 네이버가 AI팩토리 운영사(OpCo)를 100% 지분 연결·고객 매출 100% 인식·SPV에 컴퓨팅 비용(1MW당 $x) 지급. SPV는 브룩필드 최대 $9B 투자로 대주주(네이버 비연결·오프밸런스), 운영사는 필요시 소수 지분. 지분 [불일치] 4소스 판정 뒤집힘 — 260727 IR "거의 100%"가 맞았고 260615 "JV 50%+1주" 채택 폐기',
        'valuation ②: Phase1·증분 귀속률 50/10% → 100/100%(시나리오 공통). B 3.7조→15.1조. 레버 표 재실측(증분확률 70% +69% / MW 200억 +68% / OPM 30% +75% / ARC Bull +178%), 결론 개정 — B 주가 기여 +5~11%p 천장 → +43%p 격상, C(ARC)는 여전히 최대 스윙(+125%p)',
        'valuation ⑥: 자금 부담 로직 재정의 — 구 "CAPEX×에쿼티×OpCo 지분" 폐기, 신규 "네이버 SPV 소수 지분(미확정 가정, 기본 10%·0~49%)" 슬라이더로 분리(B 귀속률과 무관). 구 "네이버 $1B+전략적투자자 $1B→OpCo 에쿼티" 서술 폐기(엔비디아 $1B는 본체 유증)',
        'valuation ⑦: 자금조달·지배구조 SVG를 260803 공식 도식으로 교체(NVIDIA $1B→본체 유증 / NAVER 100%→운영사 / 운영사↔SPV 컴퓨팅 비용·파워 / Brookfield 최대 $9B→SPV 대주주). 리스크 체크 ④ "연결매출 착시" → "SPV 지분법 노출"로 교체. 이벤트 표에 260803 구조안 공개 행 추가',
        '유지: MW당 매출 150억(Base)·OPM 20%·EV/EBITDA 9배·확률 100/45%·A 앵커 35조·C(ARC) 전 파라미터·CRWV 앵커. 엔비디아 역할 축소 [불일치]는 미해소 유지(260803 도식에도 백스톱 부재)',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 SOTP에 옵션 구조 분해 추가 — 본업+B(실체 층)만 +10%, C(ARC)는 +11%p짜리 공짜 콜옵션',
      source: '자체 정리(레버 분석 후속)',
      changes: [
        'valuation: SOTP 요약에 「옵션 구조 분해」(동적) 신설 — C 제외 시 38.7조·24.3만원·+10%(본업 시장검증 + B 계약 임박의 실체 층) / C 4.05조는 옵션(+11%p, 확률 45% 가중)',
        '프레임 명문화: 현 시총 35.3조 = 본업 35.0 + 0.3조 → 시장은 B·C 7.7조의 약 4%만 반영. 다운사이드는 본업 앵커가 지지, B 실현만으로 +10%, C는 현재가에 공짜로 딸려오는 콜옵션(Bull 행사 시 +145%). "스윙은 C" 표현을 "C 베팅"이 아닌 "모니터링 우선순위"로 교정',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 ② 네오클라우드 심화 사다리 추가 — 현 구조는 이미 네오클(셸·GPU 임차형), 자산 소유 3계단마다 업사이드·자본 동반 상승',
      source: '모델 슬라이더 실측 · SKT 노트 마진 벤치 · CRWV 앵커 · 자체추정',
      changes: [
        'valuation ②: 「네오클라우드 심화 사다리」 표 신설 — ①현행 임차형 +21% / ②+SW스택(OPM30) +26% / ③+셸소유 ~+26~31% / ④+GPU소유(OPM40) ~+31% / 참고상한 CRWV 완전체 69.2조(이론치). 개념 교정: 현 구조는 콜로가 아니라 부동산만 임차한 네오클라우드(GPUaaS)',
        '핵심 명문화: 업사이드↑ = 자본 기하급수↑(GPU 520억/MW). 현 구조의 낮은 업사이드는 자본을 안 대는 선택의 가격. IR "네오클 전환 2028년 이후"가 갈림길 — 자본 투입 여부가 B 리레이팅 트리거',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 — 글로벌 PSP 커버리지(260723)로 가정 교차검증: 셸 150억/MW 삼중 검증·비용 스택 분해 최초 확보 (수치 변경 없음)',
      source: '글로벌 IB PSP(전 비트코인 마이너 AI DC 전환) 커버리지 260723 · 자체 정리',
      changes: [
        '검증 통과 4건: 셸 구축비 150억/MW(딜 실측 $10~12/W=150~180억, 각 세종 163억과 삼중 정합) · 증분 확률 45%(미계약 파이프라인 벤치 50%) · B raw $18/W(딜 가치창출 $15~19/W와 정합) · 초과수요 테제 일치',
        'valuation ⑥: 셸 단가 근거를 자체추정 → 삼중 검증으로 승격. ② B 로직: 비용 스택 분해 최초 확보 — MW당 120억 = 콜로 셸 임차 28~36억($1.86~2.37/W/yr 실측) + GPU 리스·운영 85~90억, 마진 20% 가정의 하부 구조 검증',
        'Phase1 확률 Base 100% 유지(사용자 결정). 참고 기록: 스트리트 벤치는 계약 임박 사이트도 Base 75% 적용 — Bear 65%가 완충',
        '결론 불변: Base SOTP 42.7조·26.8만원·+21%. 재점검의 효과는 수치가 아니라 가정 방어력 — 세종 자체 구축 시나리오(6.6조·FCF 55%)와 B 마진 20%가 외부 실측 근거 획득',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 ② 1GW 수취액·레버 분석 추가 — 연 귀속 0.54조(매출의 3.6%), 스윙은 B가 아니라 C(ARC)',
      source: '모델 슬라이더 민감도 실측 · 자체추정',
      changes: [
        'valuation ②: 「1GW 수취액·레버 분석」 블록 신설 — 투입 대비 수익률(출자 2.85조→연 0.54조 = 에쿼티 ~19%/년, 유증 상쇄 실질 1.35조 기준 ~40%/년) + 레버 4종 실측 표(증분지분 50% 시 +32% / MW 200억 +24% / OPM 30% +26% / ARC Bull +145%)',
        '결론 명문화: B(GPU 임대)는 지분 협상을 이겨도 주가 기여 +5~11%p 천장. 유일하게 지분 희석 없는 C(ARC)가 최대 스윙(+124%p). 1GW의 전략적 의미 = ARC가 올라탈 인프라를 남의 돈 ~97%로 확보. 투자 논거 무게중심은 수주공시보다 ARC 외부고객 트랙션',
        'valuation ③: ARC desc에 "지분 희석 없는 100% 네이버 몫 — SOTP 최대 스윙 레이어" 명시. 미반영 상방 2건(1GW 초과 글로벌 확장·학습 캐파 내재화) 기록',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 A를 시장가치 앵커 35.0조로 하향 — 5-Part 이론가 45.54조는 참고로 강등 → Base SOTP 53.3조→42.7조(26.8만원, +21%)',
      source: '사용자 지시 · AIDC 부각 전 본업 시총(~35조) 앵커 · 자체추정',
      changes: [
        'valuation ①: 「본업 밸류 A — 시장가치 앵커」 슬라이더 신설(기본 35.0조, 30~55조). A 채택값 = 앵커, 5-Part 합계(45.54조)는 이론가 참고로 병기(−23% 할인 = 리레이팅 리스크·컨글로머릿 할인). 5-Part 슬라이더·AIDC 분리 로직·파이낸셜 각주는 구성 근거로 전부 유지',
        '결과: Base SOTP 53.3조→42.7조(+51%→+21%), 적정주가 33.4만→26.8만원. Bear 36.3조(22.8만원, +3%) · Bull 94.5조(59.3만원, +168%)',
        '참고: A를 5-Part 이론가(45.54조)로 되돌리면 Base 53.3조·33.4만원 — 리포트 TP 33만원과 수렴하는 케이스는 참고 시나리오로 보존(슬라이더 조절)',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 ⑦ 돈 흐름·장소 구분 정정 — 엔비디아는 순수취 +$8B(내주는 쪽 아님) · 세종 ≠ Phase1 200MW(콜로 임차)',
      source: '260727 IR 통화 Q10·11·13·21 · 260727 공시(유증·기재정정) · 자체 정리',
      changes: [
        'valuation ⑦: 「돈 흐름 — 누가 내고 누가 받나」 신설 — 브룩필드 $9B→SPV→GPU 대금→엔비디아 수취 / 엔비디아 $1B→네이버 유증(공사비 아닌 지분 매입대금) / 네이버·전략투자자 각 $1B→OpCo 에쿼티 / 운영기 고객→OpCo→SPV→대주단. 정산 시 엔비디아 순수취 +$8B — 공시 정정("GPU 공급의 주체"로 축소)·서큘러 딜 구조와 정합',
        'valuation ⑦: 「장소 구분 — 세종 ≠ 200MW」 신설 — Phase1 200MW는 타사 콜로 임차(사업자·지역 비공개, 셸·전력 콜로 부담), 세종은 증분 구간 2029~30 자체 증설분(로드맵 200~300MW). 공시 "각 세종 첫 거점"은 사업 거점 서술',
        'valuation ⑥: 자체 구축 시나리오에 "Phase1 200MW 무관 — 세종은 증분 구간" 명시',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 ⑥ 자금부담 섹션 확장 — 자금 사다리·에쿼티 해설·손익 귀속 흐름·리스크 체크 4·자체 구축 슬라이더 (기본값 결과 불변)',
      source: '260615 설명회 구조 · 각 세종 1차 투자(6,500억/40MW) · 자체추정',
      changes: [
        'valuation ⑥: 자금 사다리(동적) 신설 — 1GW 총 CAPEX → 부채·PF 80% → 에쿼티 20% → 네이버 출자로 좁아지는 구조 시각화 + 연간 부담 vs FCF 게이지. 반론("FCF 2조로 82조를 어떻게") 대응용: 질문을 연 부담액으로 전환',
        'valuation ⑥: 「지분 10%의 의미」 해설 — 지분율은 에쿼티에 대한 비율(부채는 소유권 없음). 에쿼티 비율 20/40/100% → 네이버 1.35/2.70/6.75조 사다리. Phase1은 50%×20%=총액 10%라 두 개념이 우연히 일치했음을 명시',
        'valuation ②: 1GW 풀가동 연간 손익 귀속 흐름(동적) — 매출 15조 − 리스료·비용 12조 = EBITDA 3조 → 네이버 귀속 0.54조(Phase1×50%+증분×10%). 연결/지분법 회계 표시 착시 경고(증분 매출 미표시·Phase1 전액 표시)',
        'valuation ⑦: 자금구조 리스크 체크 4 — ①임차계약=사실상 신용보강 ②파트너 에쿼티 90% 미확정 ③엔비디아 이중지위(서큘러 딜) ④연결매출 착시',
        'valuation ⑥: 자체 구축 슬라이더 2종 신설 — 자체 MW(0~600, 기본 0)·셸 단가(80~250억/MW, 기본 150=각 세종 1차 163억 정합). 켜면 총부담·연간·FCF%에 반영(세종 250MW 시 6.6조·FCF 55%). 기본 0이라 기존 결과(출자 2.85조·FCF 24%) 불변. 감가상각→A 침식은 note만(자동 연동 안 함)',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 SOTP 가정 갱신 — Phase1 확률 100%(Base·Bull) · 기존 클라우드 PSR 4.5x→세후이익×PER 20배 · 파이낸셜 두나무 교환지연 각주 · 260615 자금구조 SVG 도식 삽입 → Base SOTP 54.7조→53.3조(33.4만원)',
      source: '사용자 가정 지시 · 증권사 리포트 2건(260710·260716) · 260615 신사업 설명회 도식 · 자체추정',
      changes: [
        'valuation ②: Phase1 실현 확률 Base 80%→100%, Bull 95%→100%(Base가 Bull을 초과하는 역전 방지 동반 상향, Bear 65% 유지). 근거[자체추정]: 전략적투자자·200MW 고객사 모두 엔트로픽으로 강추정 — 260615 "엔트로픽·구글·xAI 계약 논의" + 260716 리포트 "단일 고객 200MW+, 미국 빅테크 추정". 반증 조건: 수주공시 고객사명',
        'valuation ①(4): 기존 클라우드 평가를 PSR 4.5x → (매출−AIDC 1,050) × OPM 15%(증권사 신규 클라우드 전망 준용) × (1−세율 24%) × PER 20배로 전환 = 1,997.3십억. AIDC는 B 레이어 EV/EBITDA 9배 유지. A 합계 47,481.8→45,537.1십억(45.54조)',
        'valuation ①(2): 파이낸셜 각주 추가 — 두나무 주식 교환 12/31로 지연 중(260710 리포트), 교환 무산·재협상 시 4,936십억 평가 하향 리스크',
        'valuation ⑦: 260615 설명회 자금조달·지배구조 SVG 도식 삽입(네이버 $1B + 전략적투자자 $1B → JV(OpCo) 50%+1주 / FI 80% → SPB(GPU 자산) / GPU 리스 / 컴퓨트 판매 → 고객. 엔트로픽 라벨은 자체추정 표기)',
        '결과: Base SOTP 54.7조→53.3조(+55%→+51%), 적정주가 34.3만→33.4만원 — 리포트 목표주가 33만원과 수렴. Bear 46.8조(29.4만원) · Bull 105.0조(65.9만원)',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 SOTP 정정 — 리포트 엔터프라이즈 2027E 1,926십억에 AIDC 포함 확인 → 혼입 차감 기본 1,050십억 ON, A 52.21조→47.48조, Base SOTP 59.4조→54.7조(34.3만원)',
      source: '증권사 리포트 2건(260710·260716) 부문별 전망 테이블 · 자체추정',
      changes: [
        '판정 근거: 엔터프라이즈 실적 2025 564.2십억·2026F ~655십억. AIDC 미반영 추정 2027F 784.7(organic +17%) vs AIDC 반영 추정 2,058.3. 리포트 1,926은 organic으로 도달 불가(+194%) → AIDC ~1.1조 포함 확실',
        '기존 "미포함 판단"의 오류 정정: "포함 시 기존 클라우드 0.88조로 반토막 역성장" 논거는 2025 기존 클라우드를 ~1.7조로 잘못 가정한 것(실제 0.56조). 1,926−1,050=876십억은 2026F 대비 +34%로 정상 성장',
        'valuation ①: 혼입 차감 슬라이더 기본값 0→1,050(본 모델 2027 AIDC ≈ 70MW×150억, 3개 시나리오 공통). (4) 8,667→3,942십억, A 합계 52,206.8→47,481.8십억. 차감 0(리포트 원값)이 참고 시나리오로 반전',
        '해석 전환: 리포트 목표주가 33만원 = 본업 + AIDC(P/S 4.5x 전액). 본 모델은 AIDC를 B 레이어(EBITDA×9배×확률 80%×지분 50% = 유효 ~0.7x)로 분리해 약 6배 보수적으로 평가',
        'MW당 매출 교차검증 추가: 260716 리포트 신규 클라우드 2030E 12.44조 ÷ 유효 ~630MW ≈ 197억/MW → Bull 200억을 지지하는 증권사 독립 추정(기존 "상단 지지는 회사 제시치뿐" 서술 보강). OPM 15% 전망은 Bear 마진과 일치',
        '결과: Base SOTP 59.4조→54.7조(+68%→+55%), 적정주가 37.3만→34.3만원. Bear 48.8조(30.6만원) · Bull 106.6조(66.9만원)',
      ],
    },
    {
      date: '2026-07-28',
      title: '네이버 SOTP 재구성 — A를 본업 5-Part SOTP 52.19조로 확장 + B에 OpCo 지분율(Phase1 50%+1주 / 증분 10%) 적용 → Base SOTP 64.9조→59.4조, 적정주가 40.7만→37.3만원',
      source: '260615 네이버 AI 팩토리 신사업 설명회 · 260727 네이버 IR 통화 · 260727 DART 공시 3건 · 리포트 SOTP 표 · 자체추정',
      changes: [
        'valuation ①: 「2026E 지배주주순이익 × P/E」 → 본업 5-Part SOTP로 전면 교체. (1)플랫폼 8,042.3십억×OPM 35%×(1−세율 24%)×P/E 16.0배=34,228.0 + (2)파이낸셜 4,936.0×69.0%=3,405.8 + (3)웹툰 1,860.0×63.3%×(1−할인 40%)=706.4 + (4)엔터프라이즈 1,926.0×P/S 4.5배=8,667.0 + (5)LY 26,788.5×64.7%×50.0%×(1−할인 40%)=5,199.6 → A 합계 52,206.8십억. 지분율·할인율은 상수 고정, 부문별 스윙 팩터만 슬라이더',
        'valuation ①: 엔터프라이즈 부문을 ①기존 클라우드 ②AIDC ③ARC로 3분해 — A(4)는 ①만 반영하고 ②는 B 레이어·③은 C 레이어에서 별도 평가(레이어 분리로 중복 없음). 2027E 매출 1,926십억은 ①만의 추정치로 판단(AI 팩토리 ~1.05조 포함 시 기존 클라우드가 반토막 역성장이라 비현실적·ARC 매출 미미). ②③ 혼입 대비 「AIDC·ARC 혼입 차감」 슬라이더 신설(기본 0, 1.05조 차감 시 Base 59.4조→54.6조)',
        'valuation ①: (1)플랫폼 34.2조는 AIDC 부각 전 본업 시장가치 ~35조와 사실상 일치하는 리레이팅 없는 앵커. A는 3개 시나리오 공통 고정이고 스윙은 B·C에서만 발생',
        'valuation ②: 네이버 OpCo 지분 슬라이더 2종 신설(Phase1 50% · 증분 10%) — 260615 설명회 도식(네이버 $1B + 전략적투자자 $1B + SPV 외부조달 $8B, JV=OpCo 50%+1주) 기준. B = 확률가중가치 × 구간 지분율',
        'valuation ⑥: 「네이버 직접 출자 비중」 → 「OpCo 에쿼티 비중(20%)」으로 개편하고 Phase1 CAPEX를 $10B 고정(총 CAPEX 증가분은 증분 구간에만 배분). 1GW CAPEX 기본값 $45B→$55B(260615 "1GW까지 $50~60B"), 시나리오 Bear $60B/Base $55B/Bull $50B',
        'valuation: OpCo 지분 [불일치] 3개 소스(260615 50%+ / 260703 비연결 / 260727 거의 100%) 병기하고 260615 채택 근거 명시. MW당 매출 문단에 260615 xAI 3배 단가 논의를 상단 지지 소스로 병기(급매 프리미엄·논의 단계라 Bull 근거는 IR 제시치 유지)',
        '결과: Base SOTP 64.9조→59.4조(+84%→+68%), 적정주가 40.7만원→37.3만원. Bear 38.1조→53.5조(33.6만원) · Bull 168.2조→111.3조(69.9만원). 1GW 출자 누계 6.7조→2.8조(FCF 대비 56%→23%)',
        '환율 가정 1,480.9 → 1,500원/$로 갱신. 공시 원문이 "총 인수대금 $1.0B 고정 · 최종 발행주식수는 납입일 4영업일 전 시장평균환율로 확정"이라 명시하므로 유증 원화액(1.4809→1.50조)·신주수(7,241,564→7,334,963주)·발행주식총수(159,318→159,411천주)도 함께 갱신. 웹툰 시총도 $1.24B×1,500=1,860.0십억으로 재환산(689.0→706.4십억). CoreWeave 앵커 시총/MW 683→692억, EV/MW 1,256→1,272억. 공시 원문 인용 수치(1조 4,809억·7,241,564주)는 ⑦ 섹션에 잠정치로 병기 보존',
        '참고: 리포트 목표주가 33만원은 본업 5-Part만 반영한 값(156,852천주 기준 적정주가 332,752원). 본 모델은 여기에 B+C 7.18조를 더하고 유증 반영 159,411천주를 적용해 37.3만원 — 차액 약 4.5만원이 Physical AI 프리미엄',
      ],
    },
    {
      date: '2026-07-23',
    ...
    },
    {
      ...
      source: '공급망 보도 · Gen3 열관리 분석노트(26.07.10) · 자체추정',
      changes: [
        'components: Gen3 양산 전환 콜아웃 — 26.6말 임원회의 설계승인(R&D→양산), 조달 지침 9월 주1,000대→연말 주2,000~2,500대, 8월분 주문 기발부 · "연 10만대"는 공급사 추산치(공식목표 아님) 경고 병기',
        'components: 옵티머스 열 아키텍처를 "공냉 기저+수냉(미세유로)" → "구조체 방열(하우징→TIM→프레임→히트파이프·vapor chamber)+열 인지 AI 제어(작업 스케줄링·부하 분산)"로 교체 · BOM 열관리 세부내역(수랭→TIM·히트파이프 등) 동기화',
        'components: Gen3 부위별 열병목 판단표(손/전완 9/10 · 무릎/엉덩이/어깨 8/10 · 인버터 7/10 등 6개 부위) + 열팽창-정밀도 콜아웃(Al 100mm·30°C 상승→~70µm ≈ 손끝 정밀도 목표 80µm 동일 오더) 신설',
        'components: RADAR_OEM·ASP표 Optimus 노트 "V3 26H2 양산 예정" → "Gen3 26.6말 설계승인·9월 주1,000대 지침" 갱신',
      ],
    },
    {
      date: '2026-07-08',
    ...
    },
    {
      ...
      changes: [
        ...
        'competition: HDS 230만 vs Leaderdrive 60만→160만(27)·옵티머스向 벤더 미정[미확인] · 실증 트래커 6사(Figure/1X/Agility/Leju/EngineAI/Agibot) · GR00T 레퍼런스+Fanuc·Galbot · 기업가치표(Figure $39B/1X $10B/Agility $3.1B/中 비상장) · 2025 출하량은 기존 시뮬레이터값(Unitree 4,200) 유지',
        ...
        'index: Per-Robot BOM Editor를 $25k 앵커에 정합 — 센서 $3,750(15%)·컴퓨트 $2,250(9%)·구조 $2,500(10%)·조립통합 $1,985 신규 추가 → 총 $25,001·액추 56.9%',
        '전탭: 기존 본문 해외 브로커 리서치명 삭제 — Goldman Sachs·China Post Securities 제거(JPM 컨퍼런스=발표 행사·McKinsey=컨설팅펌은 유지)',
      ],
    },
    {
      date: '2026-06-19',
    ...
    },
    {
      ...
      source: '현대모비스 Corp Day(2026.05.28) · 각사 IR · 언론 종합 / 자체정리 [자체추정]',
      changes: [
        'components: Atlas 조인트맵(OEM_SPECS.atlas) 유성 일원화 — 목·어깨·팔·허리·엉덩이·무릎·발목 사이클로이드/하모닉 → 유성(바디 유성 2종·모비스 내재화), 손목·손은 하모닉 유지 · 감속기 3종 박스 채택처 정정(06-09 정정이 §1.2 표에만 반영됐던 누락분 보완)',
        'components·valuation: 손/그리퍼 감속기 2개 시나리오 병기 — A) 하모닉(에스비비 노출) / B) 볼·리드 스크루(하모닉 노출 0), 둘 다 미공개 추정',
        'components: 로봇 열관리 공냉식 섹션 신설 — 전기차(수냉) vs 로봇(공냉) · 아틀라스 vs 옵티머스 아키텍처 · 액추에이터=열관리 부품사(DLC·TIM·GaN·PCM)',
        'valuation ②: Atlas 원가 BOM(회전형 액추에이터 31개×$561) + 현대모비스 아틀라스向 매출추정(2027 6,170억→2030 1.36조·납품 60%) 표 추가 [자체추정]',
        'kr_valuechain·technology: 삼현(437730) 종목카드 추가(네오오토 최근접 comp·Proto 수주) + 로봇 액추에이터 수주 4단계(Prospect→Access→Spec-in→수주확정) 사다리 + 만도·모비스 감속기 M&A·JV 내재화 벽 주석',
        'framework: 부품 수 비교(내연 3만/EV 1.2~1.5만/로봇 5천~1만)·로봇 단가 하락 경로($5만→$3만) 추가',
      ],
    },
    {
      date: '2026-06-16',
    ...
    },
    {
      ...
      source: '로보티즈 260612 현장탐방(robotis_note.html 팀 공유 노트) · 회사 공시(한경에이셀 집계) · 자체추정',
      changes: [
        'company_tam: 로보티즈 실적·2030E TAM 카드 신규 — 2025 매출 389억(+30%·흑전)·순현금 2,284억 + 2030E 보수/기본/낙관(0.44/1.25/2.64조, 자체추정)',
        'valuation: 로보티즈 밸류에이션 카드 신규 — Trailing PER~900x·PBR15.4 + 현시총÷2030E 순익 ~26배(기본 OPM18% 가정)',
        'technology: STM×NVIDIA(2026.03.16) Sim2Real 상용화 = 로보티즈 액추에이터별 Sim2Real 희소 해자 약화 옐로플래그 추가',
        'kr_valuechain: 로보티즈 감속기 표기 정정 — 유성 → 사이클로이드(DYD)·유성 QDD 하이브리드 / 우즈벡 26.4Q 가동 / 시총 5.94→4.84조',
        '확정 수치: 우즈벡 2026 4Q 가동 · 데이터 비중 궁극 50% · 올해 51만대/잔고 40만 · 합산 1.5조 목표 2030년',
        'competition: 로보티즈 액추에이터 역량 매트릭스(vs Unitree·MyActuator·mjbots) + 한국 부품사(만도·모비스·LG전자·LG이노텍·삼성전기) 진입역량 표 추가',
        'components: Dexterous Hand 원가·내구 비교(로보티즈 800만원·핑거 18만원 교체 vs 텐던 통째 교체, 글로벌 $65k/$16.5k/$5.5k) + 부위별 액추에이터 방식 표 추가',
        '26Q1 검증: 영업손실 -118.4억은 자기주식 보상 117.8억(일회성·판관비) 거의 전부 — 영업 훼손 아님(언론종합)',
      ],
    },
    {
      date: '2026-06-09',
      title: 'Atlas 감속기 = 유성 2종(모비스 내재화) 정정 · 네오오토 기술력 재기술 · 밸류에이션 ③ 재설계',
      source: '현대모비스 Corp Day(2026.05.28) · 현대차 Corp Day(2026.05.27) · 기아 NDR · 네오오토 사업보고서(2026.03.19)·분기보고서(2026.05.15)·언론종합 / 자체정리',
      changes: [
        'valuation·components: Atlas 바디 액추에이터 = 유성(planetary) 2종(토크 차이)·모비스 기어 내재화로 정정 — 기존 하모닉+사이클로이드 전제 폐기(손/그리퍼는 별도·미공개)',
        'valuation ③ 재설계: 2층 구조(바디 유성 모비스 내재화 + 한국 외부사) — 네오오토 유성 부분품(모비스 외주) · 에스비비 손그리퍼 하모닉(미공개) · 에스피지 Atlas 미채택(타로봇)',
        'valuation ②: 대당 액추에이터 31개(손가락 제외) 정정 · 액추에이터=Atlas 원가 ~60%·ASP 서비스 포함가 주석',
        'kr_valuechain·technology·components: 네오오토 정정 — "로봇용 하모닉 한계 극복 특허"(공시 미확인) 삭제 → 유성기어 EV 2단변속기 IP·美특허·모비스 인휠감속기 협업·로봇팔 관절 소형감속기·AMR 구동모듈·로봇부품 정관추가(26.3)·가동률 84%',
        'kr_valuechain·valuation: 네오오토 예산 제4공장(2026.6 준공·CAPA 940억·기어가공 7,452㎡·로봇 액추에이터 감속기 정밀부품 생산거점) + 생산거점 국내 단일/북미 현대트랜시스 경유 간접공급(자체 해외공장 없음)',
      ],
    },
    {
      date: '2026-06-04',
      title: '현대차·NVIDIA AI 기술센터 새만금 · 젠슨 황 방한 일정 확정 · 빅테크 RMAC 지분 투자 가능성',
      source: '언론 종합 · 각사 공시 · 자체정리',
      changes: [
    ...
    {
      date: '2026-06-08',
      title: 'Naver IR 교차검증 — NVIDIA vs Google 풀스택 전쟁 · Physical AI 데이터 지형 · 레이어 간 부가가치 경쟁',
      source: 'Naver IR (2026.06.08) / 자체정리',
      changes: [
        'data.js: Layer 3 kr 업데이트 — 네이버클라우드 AI 팩토리 200MW→1GW 스케일 반영',
        'data.js: Layer 4 kr 업데이트 — 네이버 (하이퍼클로바X · Nemotron LLM 연합)',
        'competition: NVIDIA vs Google 풀스택 전쟁 카드 신설 — 5개 레이어 양방향 침투 + 연합군 전략',
        'physical_ai: Physical AI 데이터 지형도 테이블 (Tesla·Google·Naver 보유 자산 비교)',
        'technology: 레이어 간 부가가치 경쟁 프레임 추가 — pricing power·수직통합·연합 전략',
      ],
    },
    {
      date: '2026-06-03',
